 ...
 * Adapted from ProtonMail WebClients packages/srp/lib/srp.ts
 */
...
import { uint8ArrayToString, mergeUint8Arrays } from './utils';
...
export const srpHasher = (arr: Uint8Array): Promise<Uint8Array> => expandHash(arr);

const littleEndianArrayToBigInteger = async (arr: Uint8Array): Promise<bigint> =>
  uint8ArrayToBigInt(arr.slice().reverse());

/**
 * Generate a random client secret.
 */
const generateClientSecret = (length: number): Promise<bigint> => {
...
export const generateProofs = async ({
  ...
  const hashedArray = await srpHasher(mergeUint8Arrays([bigIntToUint8Array(generator, 'le', byteLength), modulusArray]));
  ...
  const clientProof = await srpHasher(mergeUint8Arrays([clientEphemeralArray, serverEphemeralArray, sharedSessionArray]));
  const expectedServerProof = await srpHasher(mergeUint8Arrays([clientEphemeralArray, clientProof, sharedSessionArray]));
